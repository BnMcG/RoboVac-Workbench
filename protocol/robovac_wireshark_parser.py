...
import struct
import MsgFromServerOuterClass_pb2
import LocalInfo_pb2
...
with open('packets.json', 'r') as packets_json:
    packets = json.load(packets_json)

    for packet in packets:
        if "data" in packet["_source"]["layers"]:
            data = packet["_source"]["layers"]["data"]["data.data"]

            try:
                cipher = AES.new(bytes(aes_key), AES.MODE_CBC, bytes(aes_iv))
                decrypted_packet = cipher.decrypt(binascii.unhexlify(data.replace(':', '')))
                ...
                try:
                    ...
                    parsed = local_server_message.ParseFromString(decrypted_packet)
